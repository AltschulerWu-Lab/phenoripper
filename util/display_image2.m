function display_image2(img,axis_handle,color_scaling,colors,mask)
...
%   Portions of the mask that are marked true are set to 0 in the displayed image
  ...
  if(~strcmpi(colors{channel},''))      
    ...
    for h=1:length(ch)
        if(isempty(mask))
           ...
           final_image(:,:,ch(h))=(final_image(:,:,ch(h))+wt(h)*img(:,:,counter)).*mask; 
        end
    end
  end
    counter = counter+1;
