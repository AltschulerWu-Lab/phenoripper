...
function edit_templateBT_Callback(hObject, eventdata, handles)
...
if(get(handles.selectSingleChannelCB,'Value')==1)
  ...
  try    
    set(handles.rootDirSelectBT,'Enable','off');
    set(handles.doneBT,'Enable','off');
    channelList=get_markers_from_dir(selectedFolder,regExp,selectedFolder);
    set(handles.rootDirSelectBT,'Enable','on');
    set(handles.doneBT,'Enable','on');
  catch
    set(handles.rootDirSelectBT,'Enable','on');
...
write_data([pathname filesep filename], metadata, myhandles.rootDir, nrChannelPerImage, markersUsed)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
lastPath=load_last_path('importtemplate');
if(~exist(char(lastPath),'dir'))
...
if(~exist(pathname,'dir'))
  ...
  return;
   ...
   save_last_path(pathname,'importtemplate');
end
...
%TODO USED ALSO IN read_data.m, SHOUDL BE EXTERNALIZED  
