function filter_gui()
    ...
    if isfield(myhandles,'filterMatrix')~=0
        backup_filter=myhandles.filterMatrix;
    else
        backup_filter_matrix=[];
    end
    ...
    set(mainFig,'CloseRequestFcn',@cancelCallBack);
    ...
    if(~isfield(myhandles,'filterMatrix') ||isempty(myhandles.filterMatrix))
        createFilterPanel(filterHandle.filter{1}.filterPanel, groupList, myhandles.grouped_metadata,1,ones(4,1));
        
    else
        createFilterPanel(filterHandle.filter{1}.filterPanel, groupList, ...
            myhandles.grouped_metadata,1,myhandles.filterMatrix(1,:));
    end
    if(isfield(myhandles,'filterMatrix'))
        for fr=2:size(myhandles.filterMatrix,1)
            addFilterCallBack([], [],myhandles.filterMatrix(fr,:))
        end
    end
    
    
    function createFilterPanel(parentPanel, groupList, groupedMetadata,filterNr,filter_row)
        
        selectedGroup=filter_row(2);
        if(filterNr==1)
            ...
            filterHandle.filter{filterNr}.filterType=uicontrol('Parent', parentPanel, 'Style', 'popup',...
                'String', 'Or|And', ...
                'Units', 'normalized', 'FontWeight', 'bold',...
                'BackgroundColor', [1 1 1], 'Position', [0.02 0 0.09 0.9],'Value',...
                filter_row(1));
        end
        ...
        filterHandle.filter{filterNr}.GroupList=uicontrol('Parent', parentPanel, 'Style', 'popup',...
            ...
            'Callback', {@groupListCallBack, filterNr,groupedMetadata},'Value',...
            filter_row(2));
        filterHandle.filter{filterNr}.GroupOperator=uicontrol('Parent', parentPanel, 'Style', 'popup',...
            'String', '=|!=', ...
            'Units', 'normalized', 'FontWeight', 'bold',...
            'BackgroundColor', [1 1 1], 'Position', [0.40 0 0.05 0.9],'Value',...
            filter_row(3));
        groupFields=getUniqueLevels(groupList{selectedGroup});
        filterHandle.filter{filterNr}.GroupFields=uicontrol('Parent', parentPanel, 'Style', 'popup',...
            'String', groupFields, ...
            'Units', 'normalized', 'FontWeight', 'bold',...
            'BackgroundColor', [1 1 1], 'Position', [0.46 0 0.30 0.9],'Value',...
            filter_row(4));
    ...
    function addFilterCallBack(src, evt,filter_row)
        ...
        if(nargin>2)
            createFilterPanel(filterHandle.filter{NrFilter+1}.filterPanel,...
                groupList, myhandles.grouped_metadata,NrFilter+1,filter_row);
        else
            createFilterPanel(filterHandle.filter{NrFilter+1}.filterPanel,...
                groupList, myhandles.grouped_metadata,NrFilter+1,ones(4,1));
        end
    ...
    function groupListCallBack(src, evt, filterNr,groupedMetadata)
        ...
        groupFields=getUniqueLevels(groupList{groupIndex});
        %         try
    ...
    function cancelCallBack(src, evt)
        myhandles=getappdata(0,'myhandles');
        if(isempty(backup_filter))
            if isfield(myhandles,'filterMatrix')~=0
                myhandles=rmfield(myhandles, 'filterMatrix');
            end
        else
            myhandles.filterMatrix=backup_filter;
        end
        
        setappdata(0,'myhandles',myhandles);
        delete(gcf);
    end
    function groupFields=getUniqueLevels(field_name)
        myhandles=getappdata(0,'myhandles');
        
        try
            groupFields=myhandles.metadata_unique_levels.(field_name);
            if isnumeric(groupFields(1))
                groupFields=uint64(groupFields);
            end
        catch
            warning('Automatic metadata calculation failed');
            metadata=myhandles.metadata;
            try
                
                %groupFields=cellfun(@(x) cell2mat(getfield(x,groupList{selectedGroup})),...
                %    groupedMetadata,'UniformOutput',false);
                groupFields=cellfun(@(x) cell2mat(getfield(x,field_name)),...
                    metadata,'UniformOutput',false);
                
            catch
                %groupFields=cellfun(@(x) getfield(x,groupList{selectedGroup}),...
                %    groupedMetadata,'UniformOutput',false);
                if(~strcmp(field_name,'Number_FG_Blocks'))
                    groupFields=cellfun(@(x) getfield(x,field_name),...
                        metadata,'UniformOutput',false);
                else
                    groupFields=myhandles.individual_number_foreground_blocks;
                    groupFields=mat2cell(groupFields,ones(length(groupFields),1),1);
                end
                % groupFields=cell2mat(groupFields);
            end
            if isnumeric(groupFields(1))
            ...
            try
                ...
                groupFields=unique(cell2mat(groupFields));
            end
            myhandles.metadata_unique_levels.(field_name)=groupFields;
            setappdata(0,'myhandles',myhandles);
        end
